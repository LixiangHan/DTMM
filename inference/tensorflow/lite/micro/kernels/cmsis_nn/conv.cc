...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (input->type == kTfLiteInt8) {
    ...
    if (filter->sparsity == nullptr) {
      ...
      buf_size = sparse_convolve_wrapper_s8_get_buffer_size(
          &conv_params, &input_dims, &filter_dims, &output_dims);
    }
    if (buf_size > 0) {
...
TfLiteStatus EvalQuantizedPerChannel(
    ...
    TfLiteEvalTensor* output, TfLiteEvalTensor* im2col, TfLiteSparsity *sparsity) {
  cmsis_nn_conv_params conv_params;
  conv_params.dilation.h = params.dilation_height_factor;
  conv_params.dilation.w = params.dilation_width_factor;
  ...
  // Modified
  const q7_t *filter_data = tflite::micro::GetTensorData<int8_t>(filter);
  if (sparsity == nullptr) {
    TFLITE_DCHECK_EQ(
    ...
    csr_conv_filters csr_filter = {
        .filter_data = tflite::micro::GetTensorData<int8_t>(filter),
        .col = sparsity->col_idx,
        .rowptr = sparsity->row_ptr
    };
    TFLITE_DCHECK_EQ(
        sparse_convolve_wrapper_s8(
            &ctx, &conv_params, &quant_params, &input_dims,
            tflite::micro::GetTensorData<int8_t>(input), &filter_dims,
            &csr_filter, &bias_dims,
            tflite::micro::GetTensorData<int32_t>(bias), &output_dims,
            tflite::micro::GetTensorData<int8_t>(output)),
        ARM_MATH_SUCCESS);
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) { 
  ...
  // Modified
  int filter_idx = node->inputs->data[kConvWeightsTensor];
  const TfLiteTensor *weights_tensor = context->GetTensor(context, filter_idx);
  TfLiteSparsity *sparsity = weights_tensor->sparsity;
