...
TfLiteStatus InitializeTfLiteTensorFromFlatbuffer(
  ...
  *result = {};
  ...
  result->sparsity = nullptr;
  const auto *src_sparsity = flatbuffer_tensor.sparsity();
  if (src_sparsity && src_sparsity->compressed_sparse_row()) {
    // TfLiteSparsity *sparsity =
    //     allocate_temp
    //         ? reinterpret_cast<TfLiteSparsity*>(
    //               allocator->AllocateTemp(sizeof(TfLiteSparsity),
    //                                       alignof(TfLiteSparsity)))
    //         : reinterpret_cast<TfLiteSparsity*>(
    //               allocator->AllocatePersistentBuffer(
    //                   sizeof(TfLiteSparsity),
    //                   alignof(TfLiteSparsity)));
    TfLiteSparsity *sparsity = reinterpret_cast<TfLiteSparsity*>(allocator->AllocatePersistentBuffer(sizeof(TfLiteSparsity), alignof(TfLiteSparsity)));
    if (sparsity == nullptr) {
      TF_LITE_REPORT_ERROR(error_reporter,
                           "Unable to allocate TfLiteSparsity.\n");
      return kTfLiteError;
    }
    
    sparsity->row_ptr = src_sparsity->compressed_sparse_row()->row_ptr()->data();
    sparsity->col_idx = src_sparsity->compressed_sparse_row()->col_idx()->data();
    sparsity->input_ch = src_sparsity->compressed_sparse_row()->input_ch();
