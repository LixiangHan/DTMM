...
if __name__ == '__main__':
    ...
    model = tf.keras.models.load_model('../weights/pretrain/resnet12_%s.h5' % args.dataset)
    ...
    for node in GRAPH:
        if node['type'] == 'conv':
            filter_filepath = os.path.join('../importance/resnet12_%s' % args.dataset,
                                           '%s_filter.npy' % node['name'])
            filterlet_filepath = os.path.join('../importance/resnet12_%s' % args.dataset,
    ...
    model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
        filepath='../weights/ptmm/resnet12_%s_%s/model' % (args.dataset, time_stamp),
    ...
    convert_to_tflite(
        ...
        '../weights/ptmm/resnet12_%s_%s/model.tflite' % (args.dataset, time_stamp))

    os.system(
        'python ../generate_compressed_tflite.py ../weights/ptmm/resnet12_%s_%s/model.tflite -o ../weights/ptmm/resnet12_%s_%s/compressed_model.tflite -c ../weights/ptmm/resnet12_%s_%s/compressed_model.h'
        % (args.dataset, time_stamp, args.dataset, time_stamp, args.dataset, time_stamp))
    print(os.path.getsize('../weights/ptmm/resnet12_%s_%s/compressed_model.tflite' % (args.dataset, time_stamp)) / 1024, 'KB')
